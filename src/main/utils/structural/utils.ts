...
export const getCompatibilities: (components: List<Component>) => List<Compatibility> = components =>
    getConstraints(components).collect(
        list(c => c.constraint === "compatibility"),
        list(c => c as Compatibility)
    )

export const option: <T>(element: T) => Option<T> = element => (element ? some(element) : none())
...
export const isInGroup: (component: string) => boolean = component => component.includes(separator)
...
export const fromSet: <T>(s: Set<T>) => List<T> = s => fromArray(Array.from(s))

export const defined: <T>(l: List<T>) => List<T> = l => l.filter(e => e !== undefined)
